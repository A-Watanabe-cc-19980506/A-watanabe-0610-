...
4. 実装の内容を確認して問題なければ、コメントを記入し「Create pull request」のボタンを押してプルリクエストの作成を完了する。  
...
5. 講師に提出した旨をSlackで伝える。(連絡がないと気づかない場合があるのでよろしくお願いします。)  


#### 再提出のフロー

1. ローカルでの作業をリモートにpushする。  
2. すでに作成してある自分のPull Requestにどのような修正を行ったかのコメントを追加する。  
**※提出の際にmergeという作業を誤って行う方がいます。よくわからないまま作業をしないよう注意してください。**
3. 講師に提出した旨を伝える。  

#### 再提出の期限

カリキュラム生の方から提出していただいたら講師がレビューを行います。  
再提出が必要な場合は、どのような修正が必要かPull Requestのコメントに記述するので、カリキュラム生の方は**いつまでに修正して再提出するか**をPull Requestにコメントして、コメントした旨を講師に連絡してください。
